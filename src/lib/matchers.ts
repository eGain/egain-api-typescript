...
import { EgainDefaultError } from "../models/errors/egaindefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | EgainDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | EgainDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new EgainDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new EgainDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
