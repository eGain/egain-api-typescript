 ...
 */
export function portalSuggestionGetSuggestion(
  client: EgainCore,
  request: operations.GetSuggestionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.Suggestion | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSuggestionRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    suggestionID: encodeSimple("suggestionID", payload.suggestionID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/portals/{portalID}/suggestions/{suggestionID}")(
    pathParams,
  );

  const query = queryJoin(
    encodeFormQuery({
      "suggestionAdditionalAttributes": payload.suggestionAdditionalAttributes,
    }, { explode: false }),
    encodeFormQuery({
      "$customAdditionalAttributes": payload.customAdditionalAttributes,
    }),
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.Suggestion$inboundSchema.optional()),
    M.nil(204, models.Suggestion$inboundSchema.optional()),
