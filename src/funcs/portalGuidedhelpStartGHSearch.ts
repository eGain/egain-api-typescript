 ...
 */
export function portalGuidedhelpStartGHSearch(
  client: EgainCore,
  request: operations.StartGHSearchRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.StartGHSearchResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.StartGHSearchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GHSearchRequest, { explode: true });
  ...
  const path = pathToFunc("/portals/{portalID}/gh/search")(pathParams);

  const query = encodeFormQuery({
    "$lang": payload.language,
    "ghCustomAdditionalAttributes": payload.ghCustomAdditionalAttributes,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.StartGHSearchResponse$inboundSchema, {
      key: "Result",
    }),
    M.jsonErr([400, 401, 403, 404, 406], errors.WSErrorCommon$inboundSchema),
    M.jsonErr(429, errors.WSErrorCommon$inboundSchema, { hdrs: true }),
