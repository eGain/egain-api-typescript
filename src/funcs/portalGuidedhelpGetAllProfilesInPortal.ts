 ...
 */
export function portalGuidedhelpGetAllProfilesInPortal(
  client: EgainCore,
  request: operations.GetAllProfilesInPortalRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.ProfileResult | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAllProfilesInPortalRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/portals/{portalID}/gh/profiles")(pathParams);

  const query = encodeFormQuery({
    "$pagenum": payload.pagenum,
    "$pagesize": payload.pagesize,
    "filter[casebaseRelease]": payload["filter[casebaseRelease]"],
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.ProfileResult$inboundSchema.optional()),
    M.nil(204, models.ProfileResult$inboundSchema.optional()),
