 ...
 */
export function portalGuidedhelpRestoreQuickpick(
  client: EgainCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.RestoreQuickpickRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    quickpickID: encodeSimple("quickpickID", payload.quickpickID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/portals/{portalID}/gh/quickpicks/{quickpickID}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GHSearchResult$inboundSchema),
