 ...
 */
export function portalArticleGetArticleEditionDetails(
  client: EgainCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetArticleEditionDetailsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    publishViewId: encodeSimple("publishViewId", payload.publishViewId, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/articles/{articleID}/editions/{publishViewId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.EditionWithContent$inboundSchema),
