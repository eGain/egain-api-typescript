 ...
 */
export function portalTopicGetchildtopics(
  client: EgainCore,
  request: operations.GetchildtopicsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.TopicTreeResult | undefined,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetchildtopicsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    topicID: encodeSimple("topicID", payload.topicID, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/portals/{portalID}/topics/{topicID}")(pathParams);

  const query = queryJoin(
    encodeFormQuery({
      "topicAdditionalAttributes": payload.topicAdditionalAttributes,
    }, { explode: false }),
    encodeFormQuery({
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.TopicTreeResult$inboundSchema.optional()),
    M.nil(204, models.TopicTreeResult$inboundSchema.optional()),
